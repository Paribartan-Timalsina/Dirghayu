...
function Doctorbook({account,contract}) {
    ...
    if (isPatient) {
      ...
      try {
        ...
        if(response.status==404){
          window.alert("you haven't selected any dates till now")
          setallDates([])
        }else{
      ...
      setName(doctorDetails[0]);
      setMajor(doctorDetails[5]);
      setPhone(BigNumber.isBigNumber(doctorDetails[1]) ? doctorDetails[1].toNumber() : doctorDetails[1])
  ...
  const handleSaveAvailability = async () => {
    ...
    const res=await fetch(`https://dirghayu-backend-final.vercel.app/doctors`,{
      ...
      body: JSON.stringify({name:name,phone:phone,major:major,date:availableDates })
  })
    .then(response => {
