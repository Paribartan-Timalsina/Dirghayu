...
function Patientbook({ account, contract }) {
  ...
  const handleChange = (e) => {
    if (!doctors) {
      window.alert("No doctors available with particular name");
      return
    }
  ...
  const getDoctorsdates = async (doctorname) => {
    try {
      const response = await axios.post(
        "https://dirghayu-backend-final.vercel.app/alldates",
        { name: doctorname },
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSaveAvailability = async (info) => {
    doctorsdates.map((dates) => console.log(dates));

    const availability = doctorsdates.find((dates) => dates == availableDates);
    if (!availability) {
      ...
      try {
        const response = await axios.post(
          "https://dirghayu-backend-final.vercel.app/bookingschemaconfirm",
          {
            doctname: doctorname,
  ...
  const handleDateSelect = async (info) => {
    ...
    try {
      const response = await axios.post(
        "https://dirghayu-backend-final.vercel.app/bookingschema",
        {
          doctname: doctorname,
          patientname: patientname,
      ...
      if (
        ...
        response.data.message ===
          "Doctor already has three appointments booked for this day"
      ) {
    ...
    <div className="wrapper10">
          ...
          : state.list.map((post) => {
              return (
                <>
                  <button
                    key={post}
                    onClick={() => getDoctordetails(post)}
                    className="doctor-btn1"
                  >
                    {post}
                  </button>
                </>
      ...
      {details && (
        <div className="docdetails">
          <p>
            Meet Dr. {details[0]}, a highly qualified and experienced{" "}
            {details[4]} doctor with a specialization in {details[5]}. With a{" "}
            {details[4]} degree from a reputed institution, Dr. {details[0]} has
            been practicing medicine for several years and has gained a wealth
            of knowledge and expertise in {details[5]}. Patients appreciate Dr.{" "}
            {details[0]}'s caring and compassionate approach to healthcare, as
            well as their ability to communicate complex medical information in
            a clear and concise manner. If you're in need of medical attention,
            you can rest assured that you're in good hands with Dr.{details[0]}.
            Contact today to schedule an appointment and start your journey
      ...
      <button onClick={handleSaveAvailability} className="btn15">
        Check Availability
      </button>
    </div>
  );
